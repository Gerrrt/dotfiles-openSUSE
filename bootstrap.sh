...
# (zsh/tmux/nvim/git) is vendored under core/ via git subtree and symlinked here.
#
# Usage:
...
# (exit 104 = capability not found). Bulk first, then per-package so a missing
# name is skipped instead of taking the rest down with it.
zypper_install() {
	local -a pkgs=("$@")
...
provision() {
	say "zypper refresh (metadata)"
	sudo zypper --non-interactive --gpg-auto-import-keys refresh

	# Upgrades are USER-driven (see README): Tumbleweed = `zdup` (zypper dup),
	# Leap = `zup` (zypper up). bootstrap only refreshes metadata so a re-run
	# stays fast and never triggers a surprise rolling upgrade mid-setup.
	if grep -qi tumbleweed /etc/os-release; then
		say "detected Tumbleweed — system upgrades use 'zdup' (zypper dup)"
	else
		say "detected Leap — system upgrades use 'zup' (zypper up)"
	fi

	# Packman (codecs; openSUSE's analog to RPM Fusion) is intentionally NOT
	# auto-added: the repo URL differs Tumbleweed-vs-Leap and it isn't needed for
	# the CLI stack. See the README "openSUSE specifics" if you want codecs.

	say "zypper packages (from install/packages.txt)"
	local -a pkgs=()
	mapfile -t pkgs < <(read_pkgs "$DOTFILES/install/packages.txt")
	zypper_install "${pkgs[@]}"
	ok "zypper packages requested: ${#pkgs[@]}"
	...
	# mise — polyglot runtime manager; activated in core/zsh/tools.zsh. Runtimes
	# are fetched separately with `mise install` (kept out of bootstrap).
	...
	# parsers locally and needs the CLI (>=0.26.1). Build it via cargo, or swap to
	# `mise use -g tree-sitter`. (On Fedora this is a dnf package; here it isn't.)
	if ! command -v tree-sitter >/dev/null && command -v cargo >/dev/null; then
		say "tree-sitter-cli (cargo)"
...
wire_links() {
	...
	# openSUSE tmux bits (netspeed iface + battery) — tmux.conf sources it with -q
	[[ -f "$DOTFILES/os/opensuse.conf" ]] && link "$DOTFILES/os/opensuse.conf" "$CONFIG/tmux/os.conf"
	# tmux plugin manager (tpm) — clone once; plugins still need one `prefix+I` pass
	...
	[[ -f "$DOTFILES/os/opensuse.gitconfig" ]] && link "$DOTFILES/os/opensuse.gitconfig" "$CONFIG/git/os.gitconfig"
	...
	say "symlinking openSUSE OS-native layer"
	link "$DOTFILES/os/opensuse.zsh" "$CONFIG/zsh/os.zsh"
...
# openSUSE has no ~/.zshenv, so this entry file also sets the env the Core modules
# expect, then sources them in the ONE correct order. Mirror of the Mac's .zshrc.

# ── XDG + env (no zshenv on openSUSE) ─────────────────────────────────────────
...
# ── Core modules + openSUSE os layer + local overrides, in canonical order ──
...
ZRC
	...
	# make zsh the default LOGIN shell — idempotent: only acts if it isn't already.
...
wire_links
ok "openSUSE bootstrap complete — open a new shell or: exec zsh"
